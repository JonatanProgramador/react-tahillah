...
export default function ListPraise() {
    ...
    return (<Box  display={'flex'} flexDirection={"column"} alignItems={'center'} >
        ...
        <List sx={{ bgcolor: 'background.paper', maxWidth: '200px', padding:0 }}>
            {praises?.map((praise)=> {
                return ( <React.Fragment key={praise.id}><ListItemButton sx={{paddingBottom:0}} onClick={()=>navigate('/showPraise/'+praise.id)} >
                    <ListItemText primary={praise.title} secondary={praise.type} />
                </ListItemButton>
                {praises.length!==praise.id?<Divider />:null}</React.Fragment>);
            })}
        </List>
    </Box>);
}